...
void AGJHPlayerController::SetupInputComponent()
{
	...
	UGJHInputComponent* TDRInputComponent = CastChecked<UGJHInputComponent>(InputComponent);
	if (IsValid(TDRInputComponent) == false)
		return;

	TDRInputComponent->BindAbilityActions(InputConfig, this, &ThisClass::AbilityInputPressed, &ThisClass::AbilityInputReleased, &ThisClass::AbilityInputHeld);
