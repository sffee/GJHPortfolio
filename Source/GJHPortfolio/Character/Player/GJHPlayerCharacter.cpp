...
void AGJHPlayerCharacter::Tick(float DeltaTime)
{
	...
	// for (TActorIterator<AGJHMonsterCharacter> Iter(GetWorld()); Iter; ++Iter)
	// {
	// 	const FVector ToDir = (Iter->GetActorLocation() - GetActorLocation()).GetSafeNormal();
	// 	
	// 	const float Dot = FVector::DotProduct(GetActorForwardVector(), ToDir);
	// 	const float Acos = FMath::Acos(Dot);
	// 	const float Angle = FMath::RadiansToDegrees(Acos);
	//
	// 	const FVector Cross = FVector::CrossProduct(GetActorForwardVector(), ToDir);
	// 	const float Dir = FVector::DotProduct(Cross, FVector::UpVector);
	//
	// 	UE_LOG(LogTemp, Warning, TEXT("Angle : %f / Dir : %f"), Angle, Dir);
	// }
}
